...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const r = await run(db, `INSERT INTO users (name,email,password_hash) VALUES (?,?,?)`, [name||'', email, hash]);
    const user = { id: r.lastID, email, role:'user', name: name||'' };
    const token = jwt.sign(user, process.env.JWT_SECRET || 'changeme', { expiresIn: '7d' });
...
router.get('/profile', authRequired, async (req, res) => {
  const db = getDb();
  try {
    const u = await get(db, `SELECT id,name,email,role,address FROM users WHERE id=?`, [req.user.id]);
    res.json({ user: { ...u, address: u.address ? JSON.parse(u.address) : null } });
  } catch (e) {
...
router.put('/profile', authRequired, async (req, res) => {
  ...
  try {
    ...
    await run(db, `UPDATE users SET name=?, address=?, updated_at=datetime('now') WHERE id=?`,
