...
router.post('/init', async (req, res) => {
  ...
  try {
    ...
    const data = initMockPayment({ order_id, amount_cents: o.total_cents, method });
    res.json(data);
  } catch (e) {
...
router.post('/mock/confirm', async (req, res) => {
  ...
  try {
    ...
    // marcar pagado
    await run(db, `UPDATE orders SET status=? WHERE id=?`, [success?'paid':'payment_failed', order_id]);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: 'Error' });
  } finally {
