...
async function buildCartPricing(db, cartId, couponCode) {
  ...
  if (items.length === 0) return { items: [], totals: { subtotal:0, discount:0, tax:0, total:0 }, coupon: null };
...
router.post('/checkout', antifraudCheck, async (req, res) => {
  ...
  try {
    ...
    if (pricing.items.length === 0) return res.status(400).json({ error: 'Carrito vacío' });
    // Validar stock
    for (const it of pricing.items) {
      const p = await get(db, `SELECT stock FROM products WHERE id=?`, [it.product_id]);
      if (it.qty > p.stock) return res.status(400).json({ error: `Stock insuficiente para ${it.name}` });
    }
    ...
    await run(db, `INSERT INTO orders (id,user_id,email,shipping_address,billing_address,status,subtotal_cents,discount_cents,tax_cents,total_cents,coupon_code,payment_method,is_guest)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [orderId, null, email, JSON.stringify(shipping||{}), JSON.stringify(billing||{}),
       decision === 'reject' ? 'rejected' : 'pending_payment',
       pricing.totals.subtotal, pricing.totals.discount, pricing.totals.tax, pricing.totals.total, pricing.coupon?.code || null, payment_method, 1]);

    for (const it of pricing.items) {
      await run(db, `INSERT INTO order_items (order_id,product_id,name_snapshot,price_cents_snapshot,qty) VALUES (?,?,?,?,?)`,
    ...
    if (decision === 'reject') {
      await notify(db, { email, subject: 'Orden rechazada por seguridad', body: `Tu orden ${orderId} ha sido rechazada por nuestro sistema antifraude.` });
      return res.status(400).json({ error: 'Orden rechazada por seguridad' });
    }
    if (decision === 'review') {
      await run(db, `UPDATE orders SET status='under_review' WHERE id=?`, [orderId]);
      await notify(db, { email, subject: 'Orden en revisión', body: `Tu orden ${orderId} está en revisión manual. Te notificaremos pronto.` });
...
router.get('/:orderId', async (req, res) => {
  const db = getDb();
  try {
    const o = await get(db, `SELECT * FROM orders WHERE id=?`, [req.params.orderId]);
    if (!o) return res.status(404).json({ error: 'No encontrado' });
    const items = await all(db, `SELECT * FROM order_items WHERE order_id=?`, [req.params.orderId]);
...
router.post('/:orderId/confirm', async (req, res) => {
  ...
  try {
    ...
    if (!o) return res.status(404).json({ error: 'No encontrado' });
    if (!success) {
      await run(db, `UPDATE orders SET status='payment_failed' WHERE id=?`, [o.id]);
    ...
    for (const it of items) {
      await run(db, `UPDATE products SET stock = stock - ? WHERE id=?`, [it.qty, it.product_id]);
    }
    const pdf = await generateInvoice(db, o.id);
    // Notificar
    await notify(db, { email: o.email, subject: 'Pago confirmado', body: `Tu pago de la orden ${o.id} fue confirmado. Factura: ${pdf}` });
